...
| 01-08-T1 | 01-08 | 2 | HITL-01 | T-01-08-06 | Block Kit card includes ticker/company-name/sector/action/size/rationale/evidence/quote/paper-banner; `build_proposal_card(proposal, account_mode, company_name, sector)` renders BOTH company_name and sector best-effort (italicized `_unknown_` when None); 14 Block Kit tests pass (13 original + 1 covering company_name+sector populated and None branches); Approve+Reject+Edit-Size+Escalate buttons; REG-01 footer; build_fill_confirmation + build_no_action_message helpers | unit | `uv run pytest tests/unit/test_slack_block_kit.py -q` | ❌ W0 | ⬜ pending |
...
| Task ID | Plan | Wave | Requirement | Threat Ref | Secure Behavior | Test Type | Automated Command | File Exists | Status |
|---------|------|------|-------------|------------|-----------------|-----------|-------------------|-------------|--------|
| 01-09-T1 | 01-09 | 3 | REG-02, STRAT-01 | T-01-09-02 | gekko init prompts agreement and requires "I agree"; aborts on rejection or passphrase mismatch; User row written with agreement_acknowledged_at; CLI commands real (init/serve/run/doctor/strategy/audit); `gekko strategy create --from-chat` reads stdin until EOF and calls `compile_strategy_from_chat` (STRAT-01); `--from-chat` mutually exclusive with `--name/--thesis/--watchlist`; passphrase vault closes _GET_PASSPHRASE() indirection | unit | `uv run pytest tests/unit/test_user_agreement.py tests/unit/test_cli_strategy_chat.py -q && uv run gekko strategy create --help \| grep -q from-chat && uv run gekko --help` | ❌ W0 | ⬜ pending |
| 01-09-T2 | 01-09 | 3 | CADENCE-02, AUTH-03 (sync-engine parity) | T-01-09-06, T-01-09-10, T-01-03-05 | APScheduler 3.x AsyncIOScheduler + SQLAlchemyJobStore wired via PRE-BUILT sync Engine (NOT a URL string); `gekko.db.engine.get_sync_engine` mirrors `get_async_engine` with the same connect-event PRAGMA key pattern; daily fire CronTrigger with IANA tz; survives restart (job persisted); tzdata Windows handling validated; passphrase NEVER in `repr(engine)` or `str(engine.url)` | integration | `uv run pytest tests/integration/test_scheduler_persistence.py tests/unit/test_db_sync_engine_no_passphrase_in_repr.py -q -m integration` | ❌ W0 | ⬜ pending |
| 01-09-T3 | 01-09 | 3 | STRAT-02, REG-01, REG-03, REG-04 | T-01-09-03, 08, 09, 10 | FastAPI dashboard routes (/, /strategies, GET `/strategies/{name}/edit`, POST `/strategies/{name}/save`, POST /trigger/{name}, /healthz, /slack/events); GET fetches latest version scoped to current_user (404 if missing); POST validates form via Strategy Pydantic, calls `next_version()`, inserts new StrategyRow scoped to current_user (REG-04), PRG-redirects back to GET (STRAT-02); paper banner + REG-01 footer on every page; queries filter by current user (REG-04); **HTMX vendored at /static/htmx.min.js (no third-party CDN); SHA-384 in VENDOR.md; SRI lint gate enforces integrity+crossorigin on any future external script; CSP meta tag defense-in-depth** | unit + integration | `uv run pytest tests/unit/test_dashboard_templates_sri.py -q && uv run pytest tests/integration/test_dashboard_strategy_edit.py -q -m integration` | ❌ W0 | ⬜ pending |
