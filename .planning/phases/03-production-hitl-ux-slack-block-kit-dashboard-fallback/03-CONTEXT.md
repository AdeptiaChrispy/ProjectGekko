...
- **D-54: Edit-size = Slack modal (slack-bolt `views_open`) with qty input + plain-language order framing + cap-based validation.** Clicking "Edit size" on the proposal card opens a Slack modal with: (1) a number input pre-filled with `tp.qty`, labelled "New quantity"; (2) a context block with plain-language framing "Current: {side} {qty} {ticker} (~${total}), Ref price: ${ref_price}, Adjust shares below — cap enforced at submit"; (3) a context block "Your change is validated against your strategy's risk caps, not against the agent's original target." Submit triggers `handle_edit_size_view_submission` which:
  1. INSERTs the dedup row (same D-41 mechanism, `action_id='edit_size'`)
  2. Validates the edited qty against strategy OrderGuard hard caps via `_check_edit_size_caps(qty, ref_price, strategy, account_equity)`. Failure: modal re-renders with plain-language bound message "That's above your max of $Z (~W shares) — pick a smaller number." No state change, no audit event. **The 2% drift check (`_drift_check`) is NOT applied to operator edits** — it remains the agent's own output-consistency guard only.
  3. On pass: write `edit_size` audit event with `{old_qty, new_qty, old_notional, new_notional}`, update `proposal.qty`, then transition `PENDING → APPROVED` and dispatch to executor — single interaction, no two-step. This is the only path that mutates `qty` on a PENDING proposal.

  **Per UAT finding 2026-06-19: operator-approved redesign.** `_check_edit_size_caps` replaced `_drift_check` as the operator-edit gate. The 2% drift check made edit-size useless — even 47→50 shares (~6% change) was rejected. The real Knight Capital defense is the absolute risk bounds: `max_position_pct * account_equity`. The filled order still flows through `OrderGuard.place_order` which re-runs `check_hard_caps` and `check_qty_price_sanity` — two independent layers remain.

  **Why modal, not inline +/- buttons:** the use case is "I want 50 shares not 47" — fractional/specific qty, not a fixed delta. Modal also makes the cap error a contained UI moment (red block in the modal) instead of a confusing card update.
...
- **Daily P&L summary** — NEW in P3. The CRON TRIGGER fires at fixed 4:30pm America/New_York post-close on every NYSE trading day (D-59 gate skips closed days). The DM is a ROUTINE category per D-48 — it goes through `_send_slack_dm_respecting_quiet_hours(category='daily_pnl')` and gets deferred to window-open if 4:30pm ET falls within the user's quiet window. For typical US quiet hours (10pm-7am), 4:30pm ET is well outside the window and the DM lands immediately. *Clarified during plan-checker iteration 1 (2026-06-17):* the "regardless" wording above describes the CRON TRIGGER's fixed firing time, NOT the DM bypass classification. Content shape is Claude's discretion at planning time, but should include at minimum: today's filled trades count, gross P&L, per-strategy realized P&L, open positions snapshot, any errors/rejections that hit the audit log today.
...
- **Edit-size uses hard-cap validation, not drift check (Plan 03-11 update).** D-54 — `_check_edit_size_caps` replaces `_drift_check` as the operator-edit gate. The 2% drift check (_drift_check / D-27) is the agent's output-consistency guard and is NOT applied when the OPERATOR deliberately resizes. The real Knight Capital defense for operator resizes is the absolute dollar cap: `max_position_pct * account_equity`. The filled order still flows through `OrderGuard.place_order` which re-runs `check_hard_caps` and `check_qty_price_sanity` independently. Edit-size cannot bypass OrderGuard; it never calls `place_order` directly (T-03-05-07 invariant preserved).
...
- **OrderGuard preflight + modify-and-resubmit on edit/approve (from Phase 3 live UAT)** — surfaced 2026-06-22 after a resize was rejected by OrderGuard `qty_price_drift` (since fixed) and the proposal dead-ended at FAILED. **Approach LOCKED (operator decision):** run OrderGuard's checks at edit/approve time BEFORE committing the transition; if it would reject, re-render the edit slider with the plain-language reason and let the operator adjust + resubmit — proposal stays PENDING, never dead-ends at FAILED. Covers ALL reject reasons (sector exposure, daily loss, max trades/day, PDT, T+1, market hours, universe), not just the position-size cap the slider already clamps. **Deferred** (operator chose follow-up over P3 gap-closure); routed to **Phase 6 (Web Dashboard & Multi-User Auth)**. Note: the slider already prevents exceeding `max_position_pct × equity`; this generalizes graceful recovery to every other OrderGuard check.
